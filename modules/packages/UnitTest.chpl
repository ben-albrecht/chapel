...
module UnitTest {
   ...
   */
    ...
    */
    proc assertEqual(first: ?firstType, second: ?secondType) throws {

      assertComparable(first, second, op='!=');
      ...
      if isTupleType(firstType) && isTupleType(secondType) {
        ...
        const arraysEqual = && reduce (first == second);
    ...
    */
    proc assertNotEqual(first, second) throws {
      if canResolve("!=",first, second) {
        if !checkNotEqual(first, second) {
          var err = "assert failed -\n'" + stringify(first) +
                           "'\n==\n'" + stringify(second) + "'";
          throw new owned AssertionError(err);
        }
      }
    }
    ...
    */
    proc assertGreaterThan(first: ?firstType, second: ?secondType) throws {
    ...
    */
    proc assertLessThan(first: ?firstType, second: ?secondType) throws {
      assertComparable(first, second, op='<=');
    ...
    pragma "no doc"
    /* Assert 2 (static and dynamic) types are comparable. Throws an
       AssertionError if not */
    proc assertComparable(first: ?firstType, second: ?secondType, param op='') throws {

      if op != '' {
        // Stupid work-around for issue #TODO
        if isRangeType(firstType) && isRangeType(secondType) {
          if firstType != secondType {
            var err = "assert failed - first element is of type " +
                      firstType:string + " and second is of type "+
                      secondType:string;
            throw new owned AssertionError(err);
          } else {
            if !canResolve(op, first, second) {
              var err = "assert failed - first element is of type " +
                        firstType:string + " and second is of type "+
                        secondType:string;
              throw new owned AssertionError(err);
        ...
        } else if !canResolve(op, first, second) {
          var err = "assert failed - first element is of type " +
                    firstType:string + " and second is of type "+
                    secondType:string;
          throw new owned AssertionError(err);
      ...
      if isTupleType(firstType) && isTupleType(secondType) {
        if firstType != secondType {
          const err = "assert failed -'" + stringify(first) +
                                  "' and '"+ stringify(second) + "' " +
        ...
        if first.shape != second.shape {
          const err = "assert failed -\n'" + stringify(first) +
                                  "'\nand\n'"+stringify(second) + "'\n" +
                                  "are not of same shape";
          throw new owned AssertionError(err);
        }
      }
      else if isBoundedRange(first) && isBoundedRange(second) {
        if firstType != secondType {
          const err = "assert failed -'" + stringify(first) +
                                  "' and '"+ stringify(second) + "' " +
                                  "are not of same type";
    ...
    */
    ...
    pragma "no doc"
    /* Function that checks whether two arguments are unequal or not. Returns
       true if arguments are not equal. */
    proc checkNotEqual(first: ?firstType, second: ?secondType) throws {
      if isTupleType(firstType) && isTupleType(secondType) {
        if firstType == secondType {
        ...
        if (firstType == secondType) && (first.shape == second.shape) {
    ...
    proc __baseAssertEqual(first, second) throws {
      if first != second {
        const errorMsg = "assert failed - '" + stringify(first) +"' != '"+stringify(second)+"'";
        throw new owned AssertionError(errorMsg);
      }
    }
    ...
    proc __baseAssertGreater(first, second) throws {
      if all(first <= second) {
        var err = "assert failed - '" + stringify(first) +
                         "' <= '" + stringify(second) + "'";
        throw new owned AssertionError(err);
      }
    }
    ...
    proc assertArrayLess(array1, array2) throws {
      if array1.rank == array2.rank {
        if array1.shape == array2.shape {
          ...
          var err = "assert failed - First element is of shape " + stringify(array1.shape) +" and Second is of shape "+stringify(array2.shape);
          throw new owned AssertionError(err);
        }
      }
      else {
        var err = "assert failed - First element is of type " + array1.type:string +" and Second is of type "+array2.type:string;
        throw new owned AssertionError(err);
      }
    }
    ...
    */
    proc assertTupleLess(tuple1, tuple2) throws {
      ...
      if firstType == secondType {
        ...
        var err = "assert failed - First element is of type " + firstType:string +" and Second is of type "+secondType:string;
        throw new owned AssertionError(err);
      }
    }
    ...
    */
    proc assertRangeLess(range1, range2) throws {
      if range1.size == range2.size {
        ...
        var err = "assert failed - Ranges are not of same length";
        throw new owned AssertionError(err);
      }
    }
    ...
    */
    proc assertStringLess(string1, string2) throws {
      if string1.size == string2.size {
        ...
        var err = "assert failed - Strings are not of same length";
        throw new owned AssertionError(err);
      }
    }
    ...
    proc __baseAssertLess(first, second) throws {
      if all(first >= second) {
        var err = "assert failed - '" + stringify(first) +"'>='"+stringify(second)+"'";
        throw new owned AssertionError(err);
      }
    }
